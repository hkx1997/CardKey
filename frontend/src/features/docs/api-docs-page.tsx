...
export function ApiDocsPage() {
  ...
  const base = cfg?.apiBasePath || "/api/v1";
  // 固定兑换密钥：文档页直接展示
  const key =
    cfg?.publicRedeemApiKey ||
    "ck_redeem_demo_fixed_key_change_me";
  ...
  const url = `${origin}${base}/public/redeem`;

  const snippets = useMemo(
    () => buildSnippets({ url, key, cat, code: codeSample }),
    [url, key, cat, codeSample],
  );
...
function buildSnippets(p: {
  ...
  // 统一 2 空格缩进的 JSON body
  const body = JSON.stringify(
    { category: p.cat, code: p.code },
    null,
    2,
  );
  const bodyOneLine = JSON.stringify({ category: p.cat, code: p.code });
  // 嵌入 Go raw string 时保持多行缩进

    ...
    <nav className={cn("space-y-1", className)} aria-label="主导航">
      {ADMIN_NAV.map((item) => (
        <NavLink
          ...
          className={({ isActive }) =>
            cn(
              "nav-active-bar group flex h-10 items-center gap-2.5 rounded-md px-3 text-sm font-normal text-muted-foreground",
              ...
              isActive &&
                "is-active bg-secondary/75 font-medium text-foreground shadow-xs",
            )
          }
        >
          {({ isActive }) => (
            <>
              <span className="flex size-5 shrink-0 items-center justify-center transition-transform duration-200 group-hover:scale-105">
                <item.icon
                  className={cn(
